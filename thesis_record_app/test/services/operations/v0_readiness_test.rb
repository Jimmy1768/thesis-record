...
class Operations::V0ReadinessTest < ActiveSupport::TestCase
  test "reports expected current v0 blockers" do
    ...
    assert_not_includes result.blockers, "thesis_status_ready_for_v0"
    assert_not_includes result.blockers, "draft_status_resolved"
    ...
    assert result.checks.fetch(:v0_approval_packet_approved)
    ...
    assert result.checks.fetch(:v0_prose_review_accepted)
    assert_not_includes result.blockers, "v0_prose_review_accepted"
    assert result.checks.fetch(:v0_public_release_review_accepted)
    assert_not_includes result.blockers, "v0_public_release_review_accepted"
    ...
    assert_not_includes result.blockers, "v0_publication_approved"
    assert result.checks.fetch(:v0_publication_date_set)
    assert_not_includes result.blockers, "v0_publication_date_set"
    assert result.checks.fetch(:v0_claim_set_approved)
    assert result.checks.fetch(:v0_forecast_set_approved)
    assert_not_includes result.blockers, "v0_claim_set_approved"
    assert_not_includes result.blockers, "v0_forecast_set_approved"
    assert_not_includes result.warnings, "paper_draft_is_archive_only"
    assert_not_includes result.warnings, "v0_claim_set_candidate_only"
    assert_not_includes result.warnings, "v0_forecast_set_candidate_only"
    ...
    assert_not_includes result.warnings, "v0_approval_packet_scaffold_only"
  end
  ...
  test "approves claim and forecast inventories only when timeline and set files agree" do
    result = Operations::V0Readiness.call
