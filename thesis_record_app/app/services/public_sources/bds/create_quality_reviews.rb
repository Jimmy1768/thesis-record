  ...
  module Bds
    class CreateQualityReviews
      ...
      def initialize(actor:)
        ...
        @review_policy = @bds_policy.fetch(:parser_design_v1).fetch(:quality_review_policy_v1)
      end
      ...
      attr_reader :actor, :bds_policy, :review_policy
      ...
      def observation_scope
        MetricObservation
          ...
          .where("quality_metadata ->> 'source_table' = ?", "bds_public_file_rows")
          .order(:id)
      end
      ...
      def review_metadata_for(observation, review_status)
        source_metadata = observation.quality_metadata
        {
          ...
          source_grain: source_metadata.fetch("source_grain"),
          source_year: source_metadata.fetch("source_year"),
          source_sector_code: source_metadata.fetch("source_sector_code"),
          source_firm_age_code: source_metadata.fetch("source_firm_age_code"),
          source_firm_size_code: source_metadata.fetch("source_firm_size_code"),
          source_cell_value_raw: source_metadata.fetch("source_cell_value_raw"),
          source_publication_flag: source_metadata.fetch("source_publication_flag"),
          source_metric_status: observation.metric_status,
          review_status: review_status,
          ...
          guardrail: "Quality review preserves BDS source-native context only; no export, prediction link, claim support, AI, Operator Node, nonemployer conversion, management-layer, transaction-cost, firm-boundary, trend, ratio, or aggregation authorization."
        }
      end
      ...
      def record_audit!(counters)
        Audit::Recorder.record_system!(
          ...
          change_summary: "Upserted #{counters.fetch(:reviews_upserted)} BDS metric quality reviews",
          reason_code: "bds_quality_review_creation",
          claim_status_effect: "unchanged",
      ...
      def validate_guardrails!(before_counts, after_counts)
        %i[prediction_links exports].each do |key|
          ...
          raise ReviewError, "BDS quality review creation changed #{key}"
        end
      end
