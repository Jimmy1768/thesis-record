  ...
  module Bds
    class MetricDefinitionScaffold
      ...
      def initialize(actor:)
        ...
        @scaffold = @bds_policy.fetch(:parser_design_v1).fetch(:metric_definition_scaffold_v1)
      end
      ...
      attr_reader :actor, :bds_policy, :scaffold

      def ensure_scaffold_allowed!
        row_load_result = PublicSources::Bds::RowLoadPolicyCheck.call
        raise ScaffoldError, row_load_result.failures.join("; ") unless row_load_result.passed
        raise ScaffoldError, "BDS source rows are not loaded" unless BdsPublicFileRow.exists?
      ...
      def metric_definitions
        bds_policy.fetch(:parser_design_v1).fetch(:metric_definitions)
      end
      ...
      def record_audit!(definition)
        Audit::Recorder.record!(
          ...
          entity: definition,
          change_summary: "Scaffolded draft-disabled BDS metric definition #{definition.key}",
          reason_code: "bds_metric_definition_scaffold",
      ...
      def validate_guardrails!(before_counts, after_counts)
        %i[metric_observations quality_reviews prediction_links exports].each do |key|
          next if before_counts.fetch(key) == after_counts.fetch(key)

          raise ScaffoldError, "BDS metric definition scaffold changed #{key}"
        end
      end

      def guardrail_counts
        {
          metric_definitions: MetricDefinition.where("key LIKE ?", "bds_%").count,
          metric_observations: MetricObservation.joins(:data_source).where(data_sources: { source_kind: bds_policy.fetch(:source_kind) }).count,
          quality_reviews: MetricQualityReview.joins(:data_source).where(data_sources: { source_kind: bds_policy.fetch(:source_kind) }).count,
          prediction_links: PredictionLink.joins(:data_source).where(data_sources: { source_kind: bds_policy.fetch(:source_kind) }).count,
