module PublicSources
  module Bds
    class QualityReviewPolicyCheck
      ...
      REQUIRED_STATUS_MAPPING = { staged_context: "reviewed_context" }.freeze
      ...
      REQUIRED_MANUAL_REVIEW_REASONS = %w[
        ...
        source_publication_flag_present
      ].freeze
      ...
      def observation_failures
        ...
        failures << "#{unsupported_status_count} BDS metric observations have unsupported review-source statuses" if unsupported_status_count.positive?

        forbidden_flag_count = observation_scope.where(forbidden_flag_sql).count
        failures << "#{forbidden_flag_count} BDS metric observations authorize prohibited computation/export/link/claim effects" if forbidden_flag_count.positive?

        missing_metadata_count = observation_scope.where(missing_metadata_sql).count
        failures << "#{missing_metadata_count} BDS metric observations are missing required quality metadata" if missing_metadata_count.positive?

        failures
      end

      def observation_scope
        return MetricObservation.none if bds_source_id.nil?

        MetricObservation.where(data_source_id: bds_source_id)
      ...
      def bds_source_id
        @bds_source_id ||= DataSource.find_by(
