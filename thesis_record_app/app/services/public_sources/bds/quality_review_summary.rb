module PublicSources
  module Bds
    class QualityReviewSummary
      ...
      def call
        policy_check = PublicSources::Bds::QualityReviewPolicyCheck.call
      ...
      def review_scope
        return MetricQualityReview.none if bds_source_ids.empty?

        @review_scope ||= MetricQualityReview
                          .where(data_source_id: bds_source_ids)
                          .where("metric_quality_reviews.review_metadata ->> 'source_table' = ?", "bds_public_file_rows")
      end

      def observation_scope
        return MetricObservation.none if bds_source_ids.empty?
      ...
      def prediction_link_count
        return 0 if bds_source_ids.empty?

        PredictionLink.where(data_source_id: bds_source_ids).count
      end
      ...
      def bds_source_ids
        @bds_source_ids ||= DataSource.where(source_kind: "census_bds_public_file").pluck(:id)
      end
    end
  end
end
